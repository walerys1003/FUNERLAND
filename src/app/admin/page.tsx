...
import { Forbidden } from '@/lib/auth/admin-guard';
...
export default async function AdminPage() {
  ...
  if (AUTH_CONFIGURED && user && !isAdmin) return <Forbidden user={user} />;
  ...
  const pendingCondolences = condolenceStore.listPending().length;
  const pendingMemories = memoryStore.listPending().length;
  const totalPendingMemorial = pendingCondolences + pendingMemories;
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {pendingReviews.length > 0 && (
            <Link
              href="/admin/opinie"
              className="text-[12.5px] px-3 py-1.5 rounded-full bg-error/10 text-error font-semibold inline-flex items-center gap-1.5 hover:bg-error/15"
            >
              <ShieldAlert className="w-3.5 h-3.5" />
              {pendingReviews.length}{' '}
              {pendingReviews.length === 1 ? 'opinia oczekuje' : 'opinii oczekuje'}
            </Link>
          )}
          {totalPendingMemorial > 0 && (
            <Link
              href="/admin/nekrologi"
              className="text-[12.5px] px-3 py-1.5 rounded-full bg-amber-50 text-amber-700 font-semibold inline-flex items-center gap-1.5 hover:bg-amber-100"
            >
              <ShieldAlert className="w-3.5 h-3.5" />
              {totalPendingMemorial} wpisów w nekrologach
            </Link>
          )}
        </div>
      </div>
          ...
          <div className="mt-4 flex flex-col items-center">
            ...
            <div className="mt-4 w-full text-[13px] space-y-2">
              ...
              <Row
                label="Queue length"
                value={
                  pendingReviews.length > 0 ? `${pendingReviews.length} pending` : 'Normal'
                }
      ...
      <div className="mt-6 card p-5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[12px] text-text-secondary">
            {pendingReviews.length} w kolejce
          </span>
        </div>
        {pendingReviews.length === 0 ? (
